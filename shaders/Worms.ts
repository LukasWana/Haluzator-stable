...
const source = `
// Worms from "Digiverse" by Fairlight
...
float spi(vec3 p, bool hole) {
    ...
    // Bass affects initial twist
    pR(p.yz, p.x * (0.4 + iAudio.x * 0.4));    
    
    float y = pModPolar(p.zy, 3.);
    
    // Mids affect sine wave amplitude
    p.z -= .09 + sin(p.z) * (0.07 + iAudio.y * 0.05);
    ...
    // Highs affect surface detail ripple
 	float l = length(p.zy) - .01 - sin(p.z) * (0.03 + iAudio.z * 0.02);
    float t_local = iTime * .3; // Use local time for this part to avoid audio sync issues
    if (hole) return l;
	return max(l, op.x - t_local + .15);
...
vec3 getObjectColor(vec3 p, vec3 n, vec2 mat, float time_in) {
    ...
    if (mat.x == 0.0) return vec3(.0, .0, .1) + vec3(0., 1.0, 1.0 + iAudio.y * 0.5) * smoothstep(0.1, .0, fract(p.y * 9.));
    // Bass affects red/magenta color. Flicker is constant speed.
    if (mat.x == 2.0) return vec3(.8 + iAudio.x * 0.2, .0, .4) + floor(.1 + fract(p.x * 14. - 3. * time_in));
