...
vec3 map( vec3 p )
{
    ...
    // Audio reaction: overall volume modulates main pattern animation speed
    float time_mod = iTime * (0.5 + iAudio.w * 0.8);

    p.xz += 0.5*sincos(1.0+time_mod+(p.y+11.0*ph)*0.8);

    vec3 p1 = p; p1.xz += 0.15*sincos(1.0*p.y-1.0*time_mod+0.0);
    vec3 p2 = p; p2.xz += 0.15*sincos(1.0*p.y-1.0*time_mod+2.0);
    vec3 p3 = p; p3.xz += 0.15*sincos(1.0*p.y-1.0*time_mod+4.0);
    ...
    // Audio reaction: bass modulates the detail ripple speed
    float bass_mod = iTime*4.0 * (1.0 + iAudio.x * 1.0);
    
    return opU( opU( vec3(h1.x-0.12,                                         ph + 0.0/3.0, h1.y), 
                     vec3(h2.x-0.12-0.05*cos( 500.0*h2.y - bass_mod), ph + 1.0/3.0, h2.y) ), 
                     vec3(h3.x-0.12-0.02*cos(2000.0*h3.y - bass_mod), ph + 2.0/3.0, h3.y) );
}
...
vec3 shade( in float t, in float m, in float v, in vec3 ro, in vec3 rd )
{
    ...
    // Audio reaction: mids modulate color palette
    vec3 col = 0.5 + 0.5*cos( m*vec3(1.4,1.2,1.0) + vec3(0.0,1.0 + iAudio.y * 0.5,2.0) );
    col += 0.05*nor;
