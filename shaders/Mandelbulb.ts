...
vec3 mb(vec3 p) {
	...
	for(int i = 0; i < 7; ++i) {
		...
		phi = asin(z.z / r) + iTime*0.1 + pow(iAudio.x, 2.0) * 0.4;
        #else
        phi = asin(z.z / r);
        #endif
 ...
 vec3 f(vec3 p){ 
	 ry(p, iTime*0.2 + pow(iAudio.w, 1.5) * 0.4);
     return mb(p); 
 } 


 float softshadow(vec3 ro, vec3 rd, float k ){ 
    if (u_quality < 0.1) return 0.5;
    const int MAX_STEPS = 50;
    int steps = int(mix(16.0, 50.0, u_quality));
...
vec3 intersect( in vec3 ro, in vec3 rd )
{
    ...
    int steps = int(mix(20.0, 48.0, u_quality));
    for( int i=0; i < MAX_STEPS; i++ )
    {
        if (i >= steps) break;
 ...
 { 
	...
	vec3 res = intersect(ro, rd);
	 if(res.x > 0.0){
		   ...
		   vec3 tc0 = 0.5 + 0.5 * sin(3.0 + 4.2 * res.y + vec3(0.0, 0.5 + pow(iAudio.y, 2.0) * 0.8, 1.0));
