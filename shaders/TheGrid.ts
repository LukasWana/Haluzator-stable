...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Audio reactive time
    float time = iTime * (1.0 + iAudio.w * 0.75);
    ...
    for(int i = 0; i < N; i++) {
        ...
        vec3 intensity = vec3((NN * INTERVAL - t) / (NN * INTERVAL)) * (1.0 + iAudio.x * 0.5); // Bass affects intensity
        col = mix(col, intensity, dirt(mod(uv * max(0.0, t) * 0.1 + vec2(.2, -.2) * time, 1.2), 3.5));
        
        t = ii * INTERVAL - mod(time + INTERVAL * 0.5, INTERVAL);
        intensity = vec3((NN * INTERVAL - t) / (NN * INTERVAL)) * (1.0 + iAudio.x * 0.5);
        // Mids affect texture z-parameter
        col = mix(col, intensity * vec3(0.7, 0.8, 1.0) * 1.3, tex(uv * max(0.0, t), 4.45 + iAudio.y * 2.0));
        ...
        // Highs affect texture z-parameter here
        col = mix(col, mix(intensity * vec3(1.), intensity * vec3(0.7, 0.5, 1.0) * 3.0, rr), tex(uv * 2.0 * max(0.0, t), 0.27 + (2.0 * rr) + iAudio.z));
