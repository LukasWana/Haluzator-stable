...
float noiseStack(vec3 pos,int octaves,float falloff){
	...
	float off = 1.0;
    // COMPATIBILITY FIX: The original code used a conditional for-loop which can fail on some drivers.
    // Unrolling the loop with 'if' statements ensures compatibility.
	if (octaves>1) {
		pos *= 2.0;
		off *= falloff;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	vec2 drag = iMouse.xy; // mouse is not used in this version
	vec2 offset = vec2(0.0); // offset is not used in this version
	//
	...
	float timeSpeed = 0.5 + iAudio.w * 1.5;
	float realTime = timeSpeed*time;
	//
	vec2 coordScaled = 0.007*fragCoord;
	vec3 position = vec3(coordScaled,0.0) + vec3(1223.0,6434.0,8425.0);

	// Audio-reactive flow & turbulence
	vec3 flow = vec3(4.1*(0.5-xpart)*pow(ypartClippedn,4.0),-2.0*xfuel*pow(ypartClippedn,64.0),0.0);
	flow.y -= iAudio.x * 3.0 * xfuel * pow(ypartClippedn, 16.0); // Bass adds downward turbulence
	ypartClippedn = max(0.0, ypartClippedn - iAudio.x * 0.2); // Bass makes flames lick down
	...
	//
	vec3 noiseCoord = (vec3(1.2,0.6,0.6)*position+timing+0.4*displace3)/1.0;
	...
	// Audio-reactive color temperature
	vec3 hotColor = vec3(1.0, 0.9, 0.3); // Yellow-white hot
	fire = mix(fire, hotColor, iAudio.y * 0.6 * f); // Mids make fire hotter
	fire = mix(fire, vec3(1.2, 0.2, 0.9), iAudio.x * 0.4 * f); // Bass adds intense orange/magenta core
	//
	// smoke
	...
	float sparkLife = min(10.0*(1.0-min((sparkGridIndex.y+(190.0*realTime/sparkGridSize))/(40.0-20.0*sparkRandom),1.0)),1.0);
	...
	if (sparkLife>0.0) {
		...
		sparks *= (1.0 + iAudio.z * 2.5); // Treble makes sparks brighter and hotter
	}
	//
