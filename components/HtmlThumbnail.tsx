...
export const HtmlThumbnail: React.FC<HtmlThumbnailProps> = ({ content, htmlSettings }) => {
    if (content.type === 'url') {
        return (
    ...
    const currentSettings = htmlSettings ? { ...defaultHtmlSettings, ...htmlSettings } : defaultHtmlSettings;
    const srcDoc = generateSrcDoc(content, true, currentSettings);

    // Set wrapper background color based on settings
    ...
    return (
        <div
            className="html-thumbnail-wrapper"
            style={wrapperStyle}
        >
            <iframe
                srcDoc={srcDoc}
