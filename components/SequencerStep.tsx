...
import type { UserVideo, UserHtml, HtmlSettings } from '../types';
...
export const SequencerStep: React.FC<SequencerStepProps> = React.memo(({
    type, index, itemKey, videoInfo, imageSrc, isModel, isHtml, htmlContent, htmlSettings, previewSrc,
    ...
    // Calculate backgroundColor for HTML overlay based on htmlSettings
    const currentHtmlSettings = htmlSettings ? { ...defaultHtmlSettings, ...htmlSettings } : defaultHtmlSettings;
    let stepBackgroundColor: string | undefined;

    if (isHtml) {
        stepBackgroundColor = currentHtmlSettings.backgroundColor;
    } else if (isVideoStep || isModel) {
        stepBackgroundColor = '#000';
    } else if (type === 'shader' && itemKey && !previewSrc) {
        stepBackgroundColor = `hsl(0, 0%, ${20 + (index/31) * 15}%)`;
    }
    ...
    if (stepBackgroundColor !== undefined) {
        stepStyle.backgroundColor = stepBackgroundColor;
    }
    ...
    return (
        ...
        >
            {isHtml && htmlContent && (
                <>
                    <HtmlThumbnail content={htmlContent} htmlSettings={htmlSettings} />
